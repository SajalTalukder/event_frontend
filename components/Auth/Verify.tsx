...
const Verify = () => {
  ...
  // dispatch for redux and router for redirect
  ...
  // handling the user status (not login , login+verified)
  useEffect(() => {
    if (!user) {
  ...
  const handleSubmit = async () => {
    ...
    const result = await handleRequest(verifyRequest, setIsLoading);
  ...
  const handleResendOtp = async () => {
    ...
    const result = await handleRequest(resendRequest, setIsLoading);
    if (result) {
      toast.success(result.data.message);
    }
  };
